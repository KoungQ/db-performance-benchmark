...
RESULT_DIR="${1:-${SCRIPT_DIR}/../results/mysql}"
CLIENTS="${CLIENTS:-10}"
...
echo "MySQL sysbench run id: ${RUN_ID}"
echo "MySQL target: ${MYSQL_HOST}:${MYSQL_PORT}/${MYSQL_DB}"
echo "Clients: ${CLIENTS}, duration: ${TIME_SECONDS}s, window: ${WINDOW_SECONDS}s"
echo ""
...
for scenario in ${SCENARIOS}; do
  ...
  while (( remaining > 0 )); do
    ...
    sysbench "${SCENARIOS_DIR}/${scenario}.lua" \
      "${MYSQL_OPTS[@]}" \
      --time="${chunk_seconds}" \
      run | {
        if [[ "${PUSH_METRICS}" == "true" ]]; then
          bash "${STREAM_SCRIPT}" mysql "${scenario}" "${RUN_ID}" "${chunk}" "${output_file}"
        else
          tee -a "${output_file}"
        fi
      }
  ...
  if [[ "${PUSH_METRICS}" == "true" ]]; then
    bash "${DELETE_SCRIPT}" mysql "${scenario}" "${RUN_ID}" || true
  fi
  echo "Sleeping ${SLEEP_BETWEEN}s between scenarios..."
  sleep "${SLEEP_BETWEEN}"
done

echo "MySQL sysbench run complete: ${RESULT_DIR}"
